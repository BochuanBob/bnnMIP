...
function testDense!(xInput, weights::Array{T, 2}, bias::Array{U, 1},
               ...
               actFunc="Sign", cuts=true) where{T<:Real, U<:Real, V<:Real, W<:Real}
    ...
    y = dense(m, x, weights, bias, upper, lower, actFunc=actFunc, cuts=cuts)
    @objective(m, Min, 0)
    optimize!(m)
    if(actFunc=="Sign")
...
testDense!(xInput, weights, bias, upper, lower, cuts=false)
testDense!(xInput, weights, bias, upper, lower, cuts=true)
